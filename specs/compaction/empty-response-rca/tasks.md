...
- [x] 1.1 Add `wsErrorReason: string | null` field to `WsObservation` interface in [packages/provider-codex/src/transport-ws.ts](../../packages/provider-codex/src/transport-ws.ts); default `null`; reset on retry attempt 2 alongside `wsObs.frameCount`
...
- [x] 1.6 Update `MapResponseStreamOptions.getTransportSnapshot` callback type in [packages/provider-codex/src/sse.ts](../../packages/provider-codex/src/sse.ts) to include `wsErrorReason: string | null`; thread it into `EmptyTurnSnapshot` construction in the flush block
- [x] 1.7 Update `EmptyTurnSnapshot` interface in [packages/provider-codex/src/empty-turn-classifier.ts](../../packages/provider-codex/src/empty-turn-classifier.ts) with `wsErrorReason: string | null`; thread it into `buildClassificationPayload()` so log payload carries it
- [x] 1.8 Update [packages/provider-codex/src/empty-turn-log.ts](../../packages/provider-codex/src/empty-turn-log.ts) data-schema field list reference; no code change required (additive field per data-schema.json)
...
- [x] 1.11 Unit test in `packages/provider-codex/src/transport-ws.test.ts` (or a new file): simulate `ws.onerror` frame=0 → assert no exception escapes wsRequest; getSnapshot() returns `wsErrorReason: "WebSocket error"`
- [x] 1.12 Unit test in `packages/provider-codex/src/sse.test.ts`: extend the existing boundary regression block — verify wsErrorReason field round-trips into JSONL log entry (numeric `wsFrameCount: 0` + string `wsErrorReason: "..."` + `causeFamily: ws_no_frames`)
