...
- `gdrive_setup` and `gdrive_mount` are built-in direct tools for per-user Google Drive setup/mount lifecycle. Their boundary is CLI-first and fixed-argv: rclone/FUSE diagnostics, setup command planning, mount, remount, and unmount are agent/daemon executable without asking the user to open a terminal; Web/browser is reserved for explicit Google OAuth consent handoff. `/api/v2/gdrive/setup/status` and `/api/v2/gdrive/setup/callback` provide the first Web handoff surface, with token exchange intentionally not completed in the initial slice.
