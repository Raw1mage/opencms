...
("how does _this_ purpose evolve at runtime") live in the per-chapter
...
graph LR
    ...
    REG["~/.config/web_registry.json<br/>entryName · publicBasePath · upstreamType<br/>primaryPort/upstreamSocket · webctlPath · access"]
    ...
    DOC["/docxmcp → UDS docxmcp.sock"]
    APPN["…/<prefix> → :PORT or UDS"]
...
| `upstreamType`   | `web_registry.json`             | `tcp` (default) or `uds` typed upstream                  |
| `primaryPort`    | `web_registry.json`             | TCP upstream port when `upstreamType=tcp`                |
| `upstreamSocket` | `web_registry.json`             | Unix socket path when `upstreamType=uds`                 |
...
- `health` endpoint probes TCP ports or UDS sockets according to `upstreamType`.
- UDS routes preserve internal-only services such as docxmcp: `/docxmcp/` is public and prefix-stripped to docxmcp's own live schema landing page, while `/docxmcp/mcp`, `/docxmcp/files/*`, and `/docxmcp/healthz` remain protected/proxied to docxmcp's streamable HTTP server over its Unix socket.
...
- **Runloop modes (peer abstractions)**:
  ...
  - _Bare / passthrough mode_ (`specs/.../bare_chat_session`, 2026-06-18): reserved `bare` agent (hidden, deny-all tools) lets a same-host caller (first consumer: cecelearn 小雞老師) borrow the daemon's account pool + Claude OAuth subscription with a **persona-free** conversation. `buildStaticBlock` zeroes every system layer except the caller's `userSystem` (mirror of the codex `driverOnlyBlock`), gated strictly on `agent.name === "bare"` so normal sessions stay byte-identical (`session/llm.ts`). Built-in tools are stripped and only the `format:json_schema` StructuredOutput tool is re-injected; continuation is one-shot (no autorun); a post-condition fail-fast throws `BARE_LAYER_INJECTION_VIOLATION` if a persona layer leaks into the assembled block (`session/prompt.ts`). Reached over the same-host unix socket (`POST /api/v2/session` → `…/message` with `agent:"bare"`, `system`, `format`, pinned `model`). **Structured output is soft on the `claude-cli` OAuth-subscription backend** — `toolChoice:required` is set but the subscription backend does not force the tool-call, so callers must parse JSON from the reply and fall back (cecelearn → Gemini) on miss. History is not persisted opencode-side beyond normal session storage; "reset on reload" is the caller's responsibility (fresh session per page). Non-persistence requirement (不落地) is satisfied caller-side, not by an opencode storage change.
  - _Freerun mode_ (`harness/freerun-mode`): stateless iteration runloop driven by a structured `ContextNode` tree at `<dataHome>/storage/freerun/<sessionId>/`. No dialog history persisted between iterations — each prompt is re-synthesized from current tree state. Compaction is bypassed (DD-10). Subagent dispatch is forbidden (DD-20 — `task` tool stripped). Engine, triggers (goal / cron / watchdog), and telemetry sink (`events.jsonl` per session) live under `packages/opencode/src/freerun/`. OpenAI-compatible freerun provider calls carry `x-opencode-mode: freerun` plus session / iteration / node headers so provider-side telemetry can correlate each stateless iteration. See `plans/harness_freerun-mode/` for design + spec.
    - **Freerun routing (plans/compaction_enrichment-ai-first DD-9/DD-10, 2026-06-12)**: the freerun verdict is centralized in `packages/opencode/src/session/freerun-resolver.ts` (`FreerunResolver.decide` pure core + `isFreerunSession` session-scoped wrapper). Decision order: per-session override `off` → not freerun; override `on` → freerun; provider config `mode: "freerun"` → freerun; model `contextLimit ≤ 128_000` → freerun (small windows have no prompt-cache dividend; stateless rebuild beats accumulate+compact); else not freerun. Resolution failure fails toward turn-based (freerun strips capabilities — wrongly entering is worse than wrongly staying out).
    - **Four consumers, one resolver (DD-10)**: `session/llm.ts` (effectiveMode → prompt-injection posture), `session/prompt.ts` (explicit arm/command handling), `session/compaction.ts` (compaction bypass), `tool/bash.ts` (privileged-command block). All four MUST reach their verdict through the resolver — partial adoption desynchronizes safety posture from the compaction bypass.
    - **Per-session override + armed gate**: `WorkflowInfo.freerunOverride` (`"on" | "off"`, optional) persisted via `Session.updateFreerunOverride`; user-facing routing entry is `/freerun on|off|clear`. Execution is separately gated by the existing `workflow.autonomous.enabled` flag through `/freerun arm [goal]` / `/freerun disarm`, strong natural activation phrases such as `開始執行 <goal>` / `接著跑`, or normal autorun trigger phrases. Freerun therefore defaults to conversation mode until explicitly armed. ContextNode remains the source of truth; `FreerunTodoProjection` mirrors the root frontier into `Todo.Info[]` so A1/A2/A3 and their execution states are visible in the normal todo sidebar.
    - Tests: `session/freerun-resolver.test.ts` (boundary 128_000 inclusive / 128_001 exclusive, override precedence, provider-tag independence).
...
### Frontend Terminal / Mobile Keyboard Boundary

- `packages/app/src/components/terminal.tsx` owns the Ghostty terminal host and the browser backing textarea attributes. Mobile terminal input must disable browser autofill/spellcheck/autocapitalize behavior and keep `inputmode="text"` / terminal-oriented enter behavior so password/account helpers do not replace shell typing semantics.
- Mobile virtual keyboards primarily change `window.visualViewport`; terminal focus/resize handling must observe `visualViewport` resize/scroll in addition to layout `window.resize`, then re-fit the terminal and keep the active terminal input scrolled into the visible bottom area. This remains a frontend UI boundary and must not alter PTY backend or WebSocket protocol.
...
- File explorer pinned folder tabs are repo-scoped durable state: `packages/opencode/src/project/project.ts` stores them in `Project.Info.fileExplorer.pinnedFolders`, `project.update` publishes `project.updated`, and `packages/app/src/context/file.tsx` reads/writes through the Project API so every session for the same repo shares one pin set instead of browser `localStorage` buckets.
...
- Assistant response Markdown rendered by `packages/ui/src/components/session-turn.tsx` may receive app-provided rich-link adapters from `packages/app/src/pages/session/message-timeline.tsx`: raster local image links can expand inline, while `opencode-file://`, `.svg`, and `.md`/`.markdown` links are routed through the session page filetab adapter so mobile and desktop share the `FileTabContent` authority.
...
- Mobile/tool file browsing in `packages/app/src/pages/session/tool-page.tsx` has its own lightweight content surface; it must mirror image/SVG preview behavior for `image/*` MIME files instead of falling through to plain text rendering. Mobile session-page file-pane rendering in `packages/app/src/pages/session.tsx` must still route active file tabs through `FileTabContent` when `view().filePane.opened()` replaces the message timeline, so `open_fileview` / image-link flows keep the same file-tab authority as desktop instead of showing review/changes fallback content.
...
- `system-manager_*` is now a built-in direct tool family registered from `packages/opencode/src/tool/system-manager.ts`: it imports the shared `packages/mcp/system-manager/src/index.ts` handler directly and bypasses MCP client/server startup for OpenCMS runtime control. The legacy `system-manager` MCP server remains a compatibility entrypoint only, is retired from App Market cards via `MCP.serverApps()`, and `resolve-tools.ts` preserves the direct built-in tool when a legacy MCP key with the same name is also present.
- `gdrive_setup` and `gdrive_mount` are built-in direct tools for per-user Google Drive setup/mount lifecycle. Their boundary is CLI-first and fixed-argv: rclone/FUSE diagnostics, rclone config materialization, mount, remount, and unmount are agent/daemon executable without asking the user to open a terminal. `gdrive_setup` reuses the current OpenCMS Google login token from `~/.config/opencode/gauth.json` and writes the current user's `~/.config/rclone/rclone.conf`; it does not start a second Drive-specific OAuth flow. `/api/v2/gdrive/setup/status` verifies rclone/FUSE/remote readiness without exposing tokens.
- Drive-scope authority lives in the **C gateway login flow** (`daemon/opencode-gateway.c`), not the daemon: `/auth/login/google` requests `openid email profile <drive>` with `access_type=offline` + `include_granted_scopes=true` (incremental consent — already-granted users are not re-prompted), and on a bound login the `/auth/google/callback` handler persists the access/refresh token to `~<user>/.config/opencode/gauth.json` via a root-delegated write+chown (same session-manager pattern as `resolve_runtime_dir`; skipped when Google omits a refresh_token so an existing good token is never clobbered). This makes the single login token double as the gdrive mount credential. `OPENCODE_GDRIVE_SCOPE` (default `https://www.googleapis.com/auth/drive`) overrides the scope. Gateway login is the only iframe-immune top-level navigation, so it is the reliable Drive-consent surface; the SPA-embedded `/api/v2/google-binding/connect` is blocked by Google's `X-Frame-Options: DENY`.
- Per-user Drive mount persistence is driven by the **per-user daemon's own `InstanceBootstrap()`** (`packages/opencode/src/gdrive/automount.ts`, dispatched from `project/bootstrap.ts`), NOT a systemd service. This is namespace-critical: the file explorer is served by the per-user daemon, which runs in its own mount namespace; a FUSE mount must be spawned _inside that same daemon process_ to be visible to the explorer. A systemd `--user` service mounts in an isolated namespace (invisible to login shells) and a systemd _system_ service mounts in PID1's root namespace (visible to login shells but NOT to the daemon's namespace) — both are wrong for explorer visibility. The mount is therefore (re)created on every daemon start/restart inside the daemon's namespace. Persistence is gated by a **per-user opt-in preference** `~/.config/opencode/gdrive.json` (`{ autoMount, mountPoint, remote }`, written automatically on the first successful `gdrive_mount`, mirroring the `mcp-apps.json` per-user-config pattern); users who never set up Drive have no preference file and bootstrap skips them silently. `gdrive_mount` polls for the mountpoint after spawning rclone (FUSE takes a few seconds) instead of checking immediately, which previously made it always report `mounted:no`. WSL2 note: the apt `rclone` 1.60.x fails mount with `cannot read /proc/mounts: expected integer`; the official rclone (≥1.74) is required.
- MCP tool outputs are normalized at that aggregation boundary before model-visible text/attachments are assembled: standard `content[]` results pass through, native/dedup `{ output, metadata }` results are converted into text content, plain JSON is serialized as text plus `structuredContent`, and missing results become structured `mcp_tool_invalid_result` errors instead of leaking adapter `result.content` TypeErrors.
- **Tool-result presentation contract** (`shapeMcpResult` + `detectEmptyShell` in `resolve-tools.ts`; plan `mcp/tool-result-presentation-contract`, 2026-06-22): the contract enforces invariant **INV-PRESENT** — a successful MCP tool call carrying real data must never hand the model an empty-shell `output`. When the joined `content[].text` is empty / whitespace-only / a `see structuredContent` placeholder **and** `result.structuredContent` carries real data, `shapeMcpResult` serializes `structuredContent` back into `output` and stamps `metadata.presentationBackfill = { reason, bytes }` (explicit + observable via `presentation: structuredContent backfilled` log, never a silent fallback). The empty-shell detector tolerates a leading `<toolName>: ` prefix because external MCP servers (e.g. docxmcp default `response_format`) emit `<tool>: ok=True; see structuredContent`. Backfill still passes through `Truncate.output` (does not bypass the budget). Native (non-MCP) tools route through `ToolInvoker.execute` and never touch this contract, preserving INV-0 byte-identical presentation. This closes the BR where docxmcp default `response_format` produced a `see structuredContent` shell that the model could not read, causing a same-round re-issue loop. Its sibling RCA (store-app lazy tool returning a raw `{content}` shape → `invalid_union`) shares the same root gap and was fixed by routing all three MCP-path callsites through `shapeMcpResult`.
- Local stdio MCP processes have an explicit spawn boundary: `McpLocalConfig` accepts `cwd`, external installed binaries still run from `/tmp` for bunfig isolation, and stderr fingerprints that indicate an OpenCMS AI runtime (`[opencode]`, `session.request.identity.selected`) fail startup instead of letting malformed MCP commands create AI run sessions.
...
- **Skill index cache invalidation is process-wide, not per-instance** (`Skill.reset` in `packages/opencode/src/skill/skill.ts`, 2026-06-22): the skill index is a single on-disk scan of `<data>/skills`, but `Skill.state()` caches it through `Instance.state(createState)` which keys the cache by `Instance.directory` (`packages/opencode/src/project/state.ts` `recordsByKey`). A multi-cwd daemon therefore holds one bucket per directory. The original `Skill.reset()` only cleared the active AsyncLocalStorage context's directory bucket, so an MCP-bundled skill projected into `<data>/skills` at MCP connect time (via `CapabilitySyncExec` → `reload: Skill.reset`, running in whatever context was active) stayed invisible to a session whose directory bucket was scanned earlier and never invalidated — the BR where `doc-workflow` was correctly projected but `skill()`/`skill_loader reload` reported `count:53` without it (glob + frontmatter `safeParse` both verified fine out-of-process). Fix: `State.resetAcrossKeys(init)` clears the given `init`'s entry in EVERY key bucket; `Instance.state` exposes `getter.resetAll()`; `Skill.reset()` now calls `resetAll()` so a single reset makes a freshly projected skill visible across all instances regardless of which directory reads next.
...
- Privileged self-update is a dedicated gateway-daemon path, not general shell privilege escalation. `packages/opencode/src/server/self-update.ts` probes `sudo -n -v` and only executes fixed argv actions for installing `/etc/opencode/webctl.sh`, installing `/usr/local/bin/opencode-gateway`, installing `/usr/local/libexec/opencms-daemon-launch`, installing the gateway/per-user systemd unit templates under `/etc/systemd/system/`, running `systemctl daemon-reload`, syncing `/usr/local/share/opencode/frontend`, and restarting `opencode-gateway.service`; every action is audited to `~/.local/state/opencode/self-update-audit.jsonl`. In `packages/opencode/src/server/routes/global.ts`, `/global/web/restart` with `targets:["gateway"]` uses this path in gateway-daemon mode: compile gateway from the resolved repo source, install fixed artifacts via non-interactive sudo, return the accepted response, then schedule the service restart. Gateway-daemon `webctl.sh restart` also forces frontend dist sync through `sudo -n` via `OPENCODE_FORCE_SUDO_FRONTEND_SYNC=1`, so deployments rely on explicit `/etc/sudoers.d/` command grants and fail fast when missing instead of trying an interactive sudo fallback. For non-gateway target restarts from WebApp/settings, the route writes handover evidence and returns the accepted response before background-spawning `webctl.sh restart --graceful`; the command may terminate/reload the serving daemon, so it must not be awaited inside the HTTP handler. Non-sudoer daemons fail fast instead of attempting best-effort fallback. Gateway-owned per-user cold start is tiered at the C gateway boundary: non-sudoer users still use the legacy gateway `fork()+setuid()` path and therefore inherit the gateway sandbox, while users listed in Linux `sudo` / `wheel` / `admin` groups are started via `systemctl start opencms-daemon-wheel@<user>.service`. That sudoer unit runs the shared launcher in Unix-socket mode and omits `ProtectSystem`, so the resulting daemon does not inherit gateway's read-only mount namespace and normal OS permissions/sudoers policy provide full-system rw semantics.
- Controlled self-restart has a durable handover boundary. `packages/opencode/src/server/restart-handover.ts` writes `Global.Path.state/restart-handover/<txid>.json` before `/global/web/restart` invokes webctl or privileged self-update actions, and also updates `Global.Path.state/restart-handover/pending.json` as the latest restart marker. Repo-level `webctl.sh restart` and `templates/webctl.sh` write the same checkpoint/pending marker before scheduling their detached restart worker, so terminal-initiated restarts are observable too. The checkpoint stores redacted reason/handover/session metadata plus validation next steps; socket close remains an unknown/pending signal until health/log evidence confirms recovery. The direct `system-manager_restart_self` tool forwards optional `sessionID` and concise `handover` fields through the shared system-manager handler and reports the checkpoint path on accepted restarts. The session frontend watches completed `system-manager_restart_self` tool parts for the scheduled signal, enters the shared `restartStatus` waiting footer through `beginRestartWait`, polls `/api/v2/global/health`, and records handled tool part IDs in `sessionStorage` so a post-reload completed tool part cannot trigger an infinite reload loop. New daemon processes close the loop by writing `Global.Path.state/daemon-startup/startup.jsonl` from `packages/opencode/src/server/daemon-startup-log.ts` after the server actually binds, then atomically mark the matching checkpoint and `pending.json` as `restart-completed` so later daemon starts do not keep attributing themselves to a stale txid: `packages/opencode/src/cli/cmd/serve.ts` records both `Server.listenUnix(socketPath)` for gateway per-user daemons and TCP `Server.listen(...)`, while `packages/opencode/src/cli/cmd/web.ts` records direct web launches. Startup records include pid/ppid/uid, launch mode, bound port/hostname or socket path, and any pending restart txid/checkpoint path. The handover completion now also carries a build-id handshake (plans/infra_build-id-handshake DD-6): webctl's prod reload injects `expectedBuildId` (from `dist/.build-id`) into the active checkpoint after the install step, and `RestartHandover.complete()` compares it against the new daemon's compiled `Installation.BUILD_ID` — recording `buildIdCheck: match|mismatch|skipped-legacy` and marking the checkpoint `restart-failed` with a `failureReason` on mismatch (no rollback, no silent rewrite to completed; a dev daemon's `"local"` never satisfies a compiled expectation). Legacy checkpoints without `expectedBuildId` complete normally with an explicit `skipped-legacy` marker.
...
- Shared system-manager session/dialog handlers are DB-backed through the daemon session API boundary: session metadata/search/subagent listing/export/handover use `/api/v2/session` and `/api/v2/session/:id`, while dialog reads use `/api/v2/session/:id/message`. These routes resolve through `Session.listGlobal` / `Session.get` / `Session.messages` and the `MessageV2` `StorageRouter`, so the direct built-in tool family does not read `storage/session/<sid>/info.json` or legacy message directories as a fallback. `get_session` is the dedicated read-only session metadata tool for title/directory/timestamps/execution identity; `rename_session` remains the dedicated metadata mutation tool. Undo/redo-style session mutations use `/session/:id/revert` and `/session/:id/unrevert` rather than editing session metadata files directly.
- Claude Code native takeover is an explicit adapter boundary, not a storage fallback. `packages/opencode/src/session/claude-import.ts` reads project-scoped Claude JSONL transcripts from the supported Claude Code project transcript convention or an explicit transcript path, deterministically normalizes user/assistant text plus bounded tool evidence, and fails fast on unsupported blocks. `packages/opencode/src/server/routes/session.ts` exposes `GET /session/import/claude` for project-scoped native transcript rows and `POST /session/import/claude` for idempotent import/delta sync. Imported takeover sessions are written only through `Session.createNext`, `Session.updateMessage`, and `Session.updatePart`, preserving Bus events and `MessageV2` storage-router authority. Large takeover imports may add deterministic assistant summary anchors plus `compaction` marker parts into the same message stream; no Claude-specific sidecar compaction store exists, and `MessageV2.filterCompacted` remains the LLM-visible anchor boundary. Provider-switch recovery in `packages/opencode/src/session/prompt.ts` treats takeover anchors as stale handoff material once a newer live user turn exists, and provider-switch compaction is non-continuing (`auto:false`) so a defensive rebind never fabricates a follow-up prompt from an imported handoff. `packages/opencode/src/session/last-model.ts` treats `claude-native-transcript` and `claude-native-transcript-anchor` as import-only identities, so a newly typed live prompt resolves a runnable model instead of dispatching to transcript metadata.
- Claude transcript normalization runs `sanitizeImportedText` (a tag-list cascade) over every imported text part. The cascade strips OpenCode's own preface scaffolding (`CONTEXT PREFACE` directive header, `<context_budget>`, `<readme_summary>`, `<cwd_listing>`, `<pinned_skills>`, `<active_skills>`, `<summarized_skills>`, `<deferred-tools>`/`<deferred_tools>`, `<attached_images>`, `<attachment_ref>`, legacy `<preloaded_context>`/`<env_context>`/`<skill_context>` envelopes, structured `<skill name="..." state="...">…</skill>` entries, `[ENABLEMENT SNAPSHOT]` blocks, "Today's date: ..." lines). A pure-preface message naturally collapses to `""` and is dropped by the existing empty-text guard in `importTranscript`; partial pollution (real prompt + appended `<context_budget>`) keeps the prompt and discards the envelope. This is prophylactic — current data shows 0 sessions tagged `agent='claude-import'` so there is no live pollution to repair.
- Tool-call request shaping is now a runtime policy boundary in addition to prompt guidance. `packages/opencode/src/session/prompt.ts` derives a compact context-budget policy input from the most recent well-formed assistant usage snapshot, and `packages/opencode/src/session/llm.ts` applies `Tweaks.toolCallBudgetSync()` after `chat.params` plugins run but before `ProviderTransform.providerOptions(...)` serializes provider options. The policy clamps `maxToolCalls` by budget status and `/etc/opencode/tweaks.cfg` keys (`tool_call_budget_enabled`, `tool_call_budget_green_max`, `tool_call_budget_yellow_max`, `tool_call_budget_orange_max`, `tool_call_budget_red_max`, `tool_call_budget_unknown_max`, `tool_call_budget_absolute_max`), sets `parallelToolCalls=false` when the effective max is 1, and never raises an explicitly lower `maxToolCalls` or an explicit `parallelToolCalls=false`. `llm.tool_call_budget` debug checkpoints record original/effective values plus status/ratio/source so runaway multi-tool turns are observable as request-shaping evidence rather than prompt-only intent.
...
### Empty-response self-heal compaction (storm-prevention gate, 2026-05-05)

The runloop detects an "empty round" — `lastAssistant.finish === "unknown" || "other"` with `tokens.input === 0 && tokens.output === 0` — and runs a parent-only self-heal. The original 2026-04-29 hotfix unconditionally called `SessionCompaction.run({observed: "empty-response"})` on first empty round, on the theory that codex hits silent server-side overflow at ~80-85% context. Without a usage gate that predicate also fired on transient SSE/network blips at low context, producing a "compaction storm" pattern (~83% of compactions in long sessions traceable to empty rounds rather than real pressure).

The path is now gated by a pure helper `evaluateEmptyResponseGate({used, window, floor})` that computes `ratio = used / window` from the most recent well-formed turn (`findContextBudgetSource`) and only returns `overflowSuspected=true` when `ratio >= Tweaks.compactionSync().emptyResponseFloor` (default 0.8 — matches the original hotfix's empirical 80-85% window). Below the floor the runloop falls through to the existing nudge path (`"?"` + budget envelope); above it the original self-heal compaction + replay-user-message flow still runs. Subagents are unaffected by DD-12.

Tweak: `compaction_empty_response_floor` (parsed from `/etc/opencode/tweaks.cfg`, hot-reloadable, ratio 0..1, default 0.8).
...
**Enrichment chain rewrite (plans/compaction_enrichment-ai-first, 2026-06-12)** — `scheduleHybridEnrichment` invariants:

- **ai_paid is the SOLE enrichment path (DD-11).** `drop_old_history` (positional head-chop: keep trailing 40%, cut at Round boundary) is fully removed — it silently destroyed the anchor head where session-early decisions/constraints live, and was the primary "post-compaction amnesia" mechanism. On ai_paid failure the anchor stays untouched (explicit `enrichment failed` recentEvent with `ai_paid_failed:<errorCode>` detail + recompress telemetry + log.warn; NO fallback, no retry storm — next gate-passing compaction retries naturally). `tweaks.fadeout.aMethod` type narrowed to `"ai_paid"`; a legacy `drop_old` value in tweaks.cfg is ignored.
- **Event consistency (plans/compaction_ai-paid-event-consistency, 2026-06-12).** `runLlmCompact`'s finally-block publish carries the attempt's REAL outcome (`meta.success = LlmCompactResult.ok`; a throw counts as false). Before this fix the meta had no `success` field and `publishCompactedAndResetChain`'s `success !== false` default recorded failed ai_paid attempts as `compaction success:true` — contradicting the paired `enrichment failed` event in the same recentEvents ring (issue 20260612_session_resume_ai_paid_compaction_timeout). The undefined→true default itself is unchanged: local kinds (narrative etc.) only publish on their success path. Invariant: within one ai_paid attempt, `(compaction.success, enrichment.status)` ∈ {(true,success),(false,failed)}. UI side: the web Q card (`session-telemetry-cards.tsx`) degrades `enrichment.failed` to warning wording ("upgrade skipped — narrative summary retained") when the most recent SUCCESSFUL compaction in the ring is narrative — same scan rule as `decideAmnesiaInjection` (skip `success !== true` events). **v2 (DD-5, busMode-differentiated timeout)**: the 30s `compaction_llm_timeout_ms` was sized for the pre-DD-13 30K-input cap era; after DD-13's full-anchor input (≥128K gate floor) every background ai_paid call deterministically timed out. `runLlmCompactInner` now selects `compaction_llm_timeout_background_ms` (default 180s, range 30s–600s) when `busMode === "hybrid_llm_background"`; foreground keeps the fast 30s fail (user is waiting). No input-size dynamic scaling (DD-6: fixed wide ceiling, revisit only if telemetry shows 180s insufficient).
- **Gate = absolute 128K floor for every provider (DD-3/DD-8).** `GeneralPolicy.shouldEnrichAnchor` is `anchorTokens >= aFloorTokens` (same shape as ClaudePolicy); the old ratio gate (25%/40% of context) is retired. `contextThresholds.aCompactTokens` unified at 128_000 (claude-cli/codex/default), env-overridable but never designed below 128K.
- **Gate input = bounded undercount compensation (plans/compaction_a-tier-gate-floor, 2026-06-12).** `ClaudePolicy.gateAnchorTokens` is `min(max(estimate, realPromptTokens − 40K), ceil(estimate × UNDERCOUNT_CAP_RATIO))` with `UNDERCOUNT_CAP_RATIO = 1.5` (module constant in `context-policy.ts`, deliberately NOT a tweak). The original recency-fadeout-tiers DD-9 floor `max(estimate, real − 40K)` was a category error: `latestRealPromptTokens` measures the WHOLE prompt (system + tail + anchor), unbounded relative to anchor size, so at cache-aware high watermark (200K+ prompt) `real − 40K` always exceeded the 128K floor and the gate fired regardless of anchor size (observed: 77.8K anchor → 3/3 ai_paid triggers, one `E_HYBRID_LLM_TIMEOUT`). The 1.5x cap covers the bounded ~1.3x chars-estimator undercount (DD-9's actual intent) without letting prompt size impersonate anchor size. GeneralPolicy stays estimate-passthrough (INV-0). Skip log adds `realPromptTokens` + `capApplied`. Tests: `atier-gate-floor.test.ts` (TV-1..TV-6, incl. the 78K incident reproduction).
- **Full-anchor input (DD-13).** The historical `LLM_INPUT_TOKEN_CAP = 30K` (commit 33dc84746) was a self-imposed cost constant, not an API limit — it discarded 60–80% of a gate-passing anchor before the LLM saw it (hidden positional drop). Now the FULL anchor body feeds the compressor; the only ceiling is defensive: `contextLimit − 50K` margin, normally never reached.
- **Essence-oriented output (DD-12).** `targetTokens = 30_000` absolute hard ceiling (not a fill target; old formula `max(5K, ctx*0.3)` yielded 300K on a 1M window). The framing prompt instructs ~5K of real decisions/constraints — aligned with fadeout `aEssenceTokens` semantics. Resulting claude A/B/C cadence: C→B at 200K, B→A at 128K, A output ~5K (cap 30K).
- **C raw-tail preservation (plans/compaction_recency-fadeout-tiers P2, 2026-06-14).** Claude narrative compaction no longer folds the newest `fadeout.bTailRounds` completed C rounds into the new B anchor. `tryLocalRedactedDialog` records `metadata.rawTailProjection` on the compaction part, and `MessageV2.filterCompacted` projects `anchor + raw C tail + true post-anchor messages`, skipping lone pre-anchor unanswered originals so user replay does not duplicate already-carried intent. Legacy anchors without this metadata keep the old bounded projection.
- Tests: `compaction-recompress.test.ts` (ai_paid success → in-place anchor upgrade + stub demote; failure → anchor untouched + `ai_paid_failed` event), `context-policy.test.ts` / `claude-context-policy.test.ts` (128K boundary).
...
- `session/llm.ts` sets `x-opencode-parent-session` / `x-opencode-subagent` opencode-side headers; `packages/provider-codex/src/provider.ts` reads them and feeds `buildHeaders()` in `packages/provider-codex/src/headers.ts`, which emits the upstream names.
...
### Codex empty-turn classifier + forensic log (specs/codex-empty-turn-recovery, 2026-05-07)

Spec slug: `codex-empty-turn-recovery`. State: `implementing` on beta, advances to `verified` after live deployment confirms 24h zero-hard-error invariant.

Empty assistant turns from the codex backend (whatever upstream cause: WS truncation mid-stream, server returning `response.completed { output: [] }`, server params triggering silent degradation per OpenHands #2797) are now CLASSIFIED, LOGGED, and recovered without ever propagating as exceptions out of the provider package (INV-01 / INV-05). The **load-bearing evidence path** is the JSONL log at `<Global.Path.state>/codex/empty-turns.jsonl` (D-2); the bus channel `codex.emptyTurn` mirrors entries as a non-load-bearing convenience.

- **Classifier** lives in `packages/provider-codex/src/empty-turn-classifier.ts` as a pure function (INV-12). Six cause families per DD-9: `ws_truncation`, `ws_no_frames`, `server_empty_output_with_reasoning`, `server_incomplete`, `server_failed`, `unclassified`. Append-only enum (INV-13).
- **Recovery action vocabulary** is closed at four values: `retry-once-then-soft-fail`, `synthesize-from-deltas` (DD-8 dormant), `pass-through-to-runloop-nudge`, `log-and-continue`. `hard-error` is permanently excluded (D-1 / INV-14).
- **Log emitter** lives in `packages/provider-codex/src/empty-turn-log.ts`. JSONL path + bus publisher are INJECTED by the runtime caller (`packages/opencode/src/plugin/codex-auth.ts::setEmptyTurnLogPath` + `setEmptyTurnLogBus`) — provider package never imports `Global.Path` / `Bus` directly (INV-16 boundary discipline; same pattern as `setContinuationFilePath`). Log-failure is silenced with a single `console.error("[CODEX-EMPTY-TURN] log emission failed: ...")` breadcrumb (CET-001).
- **Hook sites**: SSE flush block (`packages/provider-codex/src/sse.ts`) when `state.emittedTextDeltas === 0 && state.emittedToolCalls.size === 0` (INV-10); replaces the historical silent `endStream()` at `transport-ws.ts:418-424` that masked WS truncation as graceful close.
- **Retry orchestration** lives in `packages/provider-codex/src/provider.ts::doStream` (WS path only): wraps the AI SDK stream, buffers the finish part, and on `recoveryAction === "retry-once-then-soft-fail"` re-opens WS for attempt 2 with `retryAttempted: true` in logContext. INV-08 cap (max 1 retry) is enforced inside `classifyEmptyTurn` itself — `snapshot.retryAttempted` demotes any further retry action to `pass-through-to-runloop-nudge`. No third retry possible regardless of caller logic.
...
- `reload`: Auto-detects dev/prod mode. Dev: kills per-user daemons (bun re-reads source on next request). Prod: hermetic binary build + build-id assertions + atomic install + kill daemons + post-restart health build-id verification.
- `restart`: Runs `reload` then recompiles and restarts gateway if source changed.
- **Build-ID handshake** (plans/infra_build-id-handshake, 2026-06-12): `script/build.ts` generates a per-build id `<git-sha-short>[-dirty].<epoch>` (DD-1), injects it as the `OPENCODE_BUILD_ID` define alongside `OPENCODE_VERSION` (DD-2), and writes a `dist/.build-id` sidecar after binary emit (DD-3). The binary self-reports via `opencode --build-id` (prints `Installation.BUILD_ID`, exit 0, no TUI); dev mode reports `"local"` (DD-8). webctl asserts twice: `_assert_build_id "post-build"` after every build (DD-4a) and `"pre-install"` before cp to `/usr/local/bin` (DD-4b) — any mismatch exits non-zero with a `build-binary --force` hint, never deploying a stale dist binary. `GET /api/v2/global/health` reports `buildId` (DD-7), and `_verify_health_build_id` polls it after prod daemon restarts, appending a `build-id ok|failed` restart event and failing the reload on persistent mismatch.
- **Hermetic build** (DD-5/DD-9): `_hermetic_build_binary()` runs `script/build.ts` under `env -i` with a whitelist PATH, per-build `mktemp` TMPDIR, and an absolute bun path. Because bun's `/tmp/bun-node-<hash>/` script shim cache ignores TMPDIR/BUN_TMPDIR (verified experimentally), `_sanitize_bun_shims()` first purges any shim dir whose `bun` symlink does not resolve to a real bun binary — the poisoned-shim failure mode (shims pointing at `/usr/local/bin/opencode` after daemon-context bun invocations) that silently broke BR#1 deploys. The post-build assertion remains the hard gate.
...
3. `webctl.sh` smart-detect dirty 層（stamp 比對）並只 rebuild 變動部分（daemon bundle / frontend bundle / gateway C binary）；frontend / backend source stamps 必須包含 tracked diff 內容與 untracked file 內容，避免同一 dirty 檔案連續修改時被誤判 unchanged；install 到系統路徑。
...
- `I-4` mobile UX collapse on large sessions: stale `#message-*` hashes are now treated as invalid anchors in `packages/app/src/pages/session/use-session-hash-scroll.ts`; when the target message is absent from the current visible user-message window, the session clears the hash, avoids `autoScroll.pause(true)`, returns to tail, and records scroll state so reconnect/hydration cannot repeatedly jump to an unloaded anchor. Valid-but-stale last-input hashes are also not replayed during ordinary initial hydration: message-hash navigation is only honored when `pendingMessage` proves an explicit navigation intent, and `packages/app/src/pages/session.tsx` clears the URL hash when the latest visible user message changes so dialog stream follow-bottom is not overridden by a prior input anchor.
