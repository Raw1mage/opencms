...
- `gdrive_setup` and `gdrive_mount` are built-in direct tools for per-user Google Drive setup/mount lifecycle. Their boundary is CLI-first and fixed-argv: rclone/FUSE diagnostics, setup approval handoff, mount, remount, and unmount are agent/daemon executable without asking the user to open a terminal; Web/browser is reserved for explicit Google OAuth consent. `/api/v2/gdrive/setup/connect` redirects to Google OAuth, `/api/v2/gdrive/setup/callback` exchanges the authorization code and writes the current user's `~/.config/rclone/rclone.conf`, and `/api/v2/gdrive/setup/status` verifies rclone/FUSE/remote readiness without exposing tokens.
