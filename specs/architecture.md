...
- Per-user Drive mount persistence is a **systemd system service** (`/etc/systemd/system/gdrive-mount.service`, `User=<user>`, `WantedBy=multi-user.target`), NOT a `--user` service: a user-manager service mounts inside an isolated mnt namespace that never propagates to login shells, whereas a system service shares PID1's root mnt namespace so the mount is visible everywhere. Requires `user_allow_other` in `/etc/fuse.conf` for `--allow-other`. WSL2 note: the apt `rclone` 1.60.x fails mount with `cannot read /proc/mounts: expected integer`; the official rclone (≥1.74) is required.
