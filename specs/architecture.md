...
- Controlled self-restart has a durable handover boundary. `packages/opencode/src/server/restart-handover.ts` writes `Global.Path.state/restart-handover/<txid>.json` before `/global/web/restart` invokes webctl or privileged self-update actions. The checkpoint stores redacted reason/handover/session metadata plus validation next steps; socket close remains an unknown/pending signal until health/log evidence confirms recovery. `packages/mcp/system-manager/src/index.ts` forwards optional `sessionID` and concise `handover` fields and reports the checkpoint path on accepted restarts.
