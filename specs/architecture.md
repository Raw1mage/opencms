...
- `system-manager_*` is now a built-in direct tool family registered from `packages/opencode/src/tool/system-manager.ts`: it imports the shared `packages/mcp/system-manager/src/index.ts` handler directly and bypasses MCP client/server startup for OpenCMS runtime control. The legacy `system-manager` MCP server remains a compatibility entrypoint only; `resolve-tools.ts` preserves the direct built-in tool when a legacy MCP key with the same name is also present.
