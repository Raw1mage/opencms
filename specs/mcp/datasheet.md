...
**Connector**: `MCP.add({type: "local", command, cwd?, env})` →
...
not part of the protocol. It is also a safety boundary: stderr lines
containing OpenCMS AI-runtime fingerprints (`[opencode]`,
`session.request.identity.selected`) indicate the child process is not
an MCP server and cause startup to fail fast.
...
Spawn cwd contract: local MCP config may provide `cwd`; absolute values
are used directly and relative values are resolved under the current
`Instance.directory`. Without `cwd`, external installed MCP binaries run
from `/tmp` to avoid project `bunfig.toml` preload leakage; all other
raw local MCP commands inherit `Instance.directory`.
