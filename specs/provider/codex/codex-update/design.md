...
Upstream codex submodule was bumped 218 commits (`5cc5f12ef..f7e8ff8e5`) in commit `dbd8f7215`. This plan brings `packages/provider-codex/` to feature parity with the subset of upstream changes that touch surfaces we mirror (Responses API client, OAuth, ChatGPT auth, wire protocol). App-server / TUI / MCP / plugin / skills internals are excluded per scope (proposal §IN/OUT).
...
| A3 | `be1d3cff93` (#20971) | `service_tier` field type in upstream Rust changed from enum `ServiceTier` to `String`. Wire values unchanged (`"priority"`, `"flex"`). | **No-op.** [types.ts:19](packages/provider-codex/src/types.ts#L19) already declares `service_tier?: string`. |
| A4 | `5d6f23a27b` (#21249) | Upstream's `/responses/compact` endpoint now propagates `prompt_cache_key` + `service_tier` from the parent request. | **No-op.** Our provider does not call `/responses/compact`. We use server-side compact via `context_management: [{type: "compaction", compact_threshold}]` ([provider.ts:80](packages/provider-codex/src/provider.ts#L80)) in the normal request, and the codex backend internally honors prompt_cache_key alignment. See DD-4. |
| A5 | `35aaa5d9fc` (#20751) | WS `ws_stream.send(...)` is now wrapped in `tokio::time::timeout(idle_timeout, …)` to bound the send side, not just the receive side. | **Adopt.** [transport-ws.ts:571](packages/provider-codex/src/transport-ws.ts#L571) sends fire-and-forget. Add a send-side timeout. See DD-3. |
...
- **DD-2** The `prompt_cache_key` source switches from `sessionId` to `threadId` (which == sessionId by default per DD-1, so no behavioral change for current callers, but semantically aligned with upstream). The custom-override path (`promptCacheKey` option in [provider.ts:64-67](packages/provider-codex/src/provider.ts#L64-L67)) is preserved. (2026-05-07)
...
| File | Change | Test |
|---|---|---|
| [packages/provider-codex/src/headers.ts](packages/provider-codex/src/headers.ts) | Add `threadId` option; emit `thread_id` header; switch `x-client-request-id` source to `threadId`; default `threadId = sessionId` when omitted (DD-1, A1) | [headers.test.ts](packages/provider-codex/src/headers.test.ts) — add cases for both-emitted, default-equality, custom-thread-id |
| [packages/provider-codex/src/provider.ts](packages/provider-codex/src/provider.ts) | Add `threadId` to `BuildHeadersOptions` plumbing; promptCacheKey now sources `threadId` (DD-2); pass through to headers builder | [provider.test.ts](packages/provider-codex/src/provider.test.ts) — extend existing prompt_cache_key cases |
| [packages/provider-codex/src/transport-ws.ts](packages/provider-codex/src/transport-ws.ts) | Wrap `ws.send` with promise + timeout race; on timeout, abort with `ws_send_timeout` reason; thread the new `wsErrorReason` value (DD-3) | [transport-ws.test.ts](packages/provider-codex/src/transport-ws.test.ts) — add a stalled-send mock case |
| [packages/provider-codex/src/types.ts](packages/provider-codex/src/types.ts) | Add `threadId?: string` to relevant request input shape (single field; not a union expansion) | covered by callers' tests |
| [packages/provider-codex/src/empty-turn-classifier.ts](packages/provider-codex/src/empty-turn-classifier.ts) | Recognize `ws_send_timeout` as a transient failure category (alongside existing `first_frame_timeout` / `mid_stream_stall`) | [empty-turn-classifier.test.ts](packages/provider-codex/src/empty-turn-classifier.test.ts) — new case |
...
1. **Unit tests**: run `bun test packages/provider-codex/` — must be green
