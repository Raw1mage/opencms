...
- [x] 1.1 Extend `BuildHeadersOptions` in [headers.ts](packages/provider-codex/src/headers.ts) with `threadId?: string`; preserve all existing fields
...
- [x] 1.5 Added 6 test cases to [headers.test.ts](packages/provider-codex/src/headers.test.ts) per TV-1..TV-5 plus a back-compat case for legacy `conversationId`
- [x] 1.6 `bun test packages/provider-codex/` → 113 pass, 0 fail
...
- [x] 2.4 `bun test packages/provider-codex/` → 115 pass / 0 fail
...
- [x] 3.8 `bun test packages/provider-codex/` → 119 pass / 0 fail
...
- [x] 4.3 `bun test packages/provider-codex/` → 121 pass / 0 fail
- [x] 4.4 plan-sync after commit (this entry)

## 5. Full provider package validation

- [x] 5.1 `bun test packages/provider-codex/` → 121 pass / 0 fail (Phase 1: 6 new + Phase 2: 2 new + Phase 3: 4 new + Phase 4: 2 new = 14 new test cases)
- [x] 5.2 `bunx tsc --noEmit -p packages/provider-codex/`: 50 errors, all pre-existing environment-level (missing `bun:test` types, `@types/node` not configured for this standalone tsconfig). Baseline unchanged from `main`. Zero regressions introduced.
...
- [x] 7.2 Fetch-back created `test/codex-update` from `main` and merged `beta/codex-update` (`458a16ec4`); validated with `bun test packages/provider-codex/` → 121 / 0
