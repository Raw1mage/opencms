...
- **DD-11**: DD-11（plugin init 鐵律）：opencode plugin factory 在 instance/plugin 初始化期間被 await，**絕不可在 factory 內 await 對 daemon 自身的 in-process client 呼叫**（如 client.config.get()）——會與 session/instance 初始化形成 reentrancy deadlock，使登入後請求 hang、反代逾時、使用者看到轉圈→404。canonical KB repo 改以**同步**解析：SPECBASE_TARGET_REPO env → 直接同步讀 opencode config 檔的 specbase.repo → input.directory。另一鐵律（DD-10 補充）：plugin 必須交付為**自包含 bundle（.js，deps inline）**，因為 compiled opencode binary 無法 runtime 解析外部 .ts 的 bare imports（@specbase/lib/markdown-it）；config.plugin 指向 dist/index.js（specbase 端 `bun run build` 產生）。
