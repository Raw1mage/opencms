...
- **A5.3 MCP tool → ToolSpec conversion** — OpenCode does the equivalent conversion in [packages/provider-codex/src/convert.ts](packages/provider-codex/src/convert.ts) using AI SDK v2's `convertTools` adapter, then emits `{ type: "function", name, description, parameters, strict }` shape. **Aligned**: yes structurally — the JSON wire shape matches C11's TEST expectations.
