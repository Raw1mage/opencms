...
- **A6.1 Compose body** — OpenCode codex-provider's `buildResponsesApiRequest` in [packages/provider-codex/src/provider.ts:62](packages/provider-codex/src/provider.ts#L62) is the analogue. **Aligned**: field-by-field matches D6-1 schema where OpenCode emits the field. **Drift**: OpenCode emits a `context_management` field for codex-server compaction inline ([provider.ts:80](packages/provider-codex/src/provider.ts#L80)) — this field is not in upstream's `ResponsesApiRequest` struct (codex-cli relies on Compact sub-endpoint, Ch09, not on inline compaction).
- **A6.2 prompt_cache_key derivation** — OpenCode emits `prompt_cache_key = sessionId` per [packages/provider-codex/src/provider.ts:81](packages/provider-codex/src/provider.ts#L81), matching upstream C2's "pure thread_id" semantics. **Aligned**: yes (post commit f5cffae25 which dropped accountId mix-in).
- **A6.3 client_metadata composition** — OpenCode emits `client_metadata = { "x-codex-installation-id": <UUID>, "x-codex-window-id": "<conv>:<gen>" }` via `buildClientMetadata` ([packages/provider-codex/src/headers.ts:108](packages/provider-codex/src/headers.ts#L108)). **Drift**: OpenCode includes `x-codex-window-id` in client_metadata; upstream **does not** (window-id is HTTP header only — D6-2). This is a real wire-shape divergence and may be a future ticket. Cache impact: backend may key on client_metadata content; extra key could mis-route or could be a no-op. Source: `specs/provider/codex-installation-id/` confirms current OpenCode behaviour.
- **A6.4 Per-turn HTTP headers** — OpenCode emits `x-codex-window-id`, `x-codex-parent-thread-id`, `x-openai-subagent`, `x-codex-turn-state`, `session_id`, `thread_id`, `x-client-request-id` via [packages/provider-codex/src/headers.ts:38](packages/provider-codex/src/headers.ts#L38). **Aligned**: yes for most. **Drift**: OpenCode emits only `session_id` + `thread_id` (underscore form); upstream emits **both** underscore AND hyphen forms (`session-id`, `thread-id`) per `build_session_headers` (D6-2 row). Backend tolerance unknown. Plus `x-codex-beta-features` not currently emitted by OpenCode (not a relevant feature surface).
