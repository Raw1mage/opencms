...
- **A8.1 Handshake headers** — OpenCode emits via [packages/provider-codex/src/transport-ws.ts](packages/provider-codex/src/transport-ws.ts) + [`headers.ts:38`](packages/provider-codex/src/headers.ts#L38) `buildHeaders` (with `isWebSocket: true`). Emits `OpenAI-Beta: <WS_BETA_HEADER>` constant + standard identity headers. **Aligned**: yes structurally. **Drift**: OpenCode's WS_BETA_HEADER value may not exactly match upstream's `responses_websockets=2026-02-06` date; worth a check during the bundle-slow-first resume work.
