...
- `@opencode-ai/provider-codex` package 瘦身為純 TS
- 消除未來誤判死碼為活碼的誘因
...
- **DD-2 保留 `packages/provider-codex/` 目錄本身.** 因為所有 `*.ts` 檔案都活著且被 `index.ts` export。只是 package 從「C + TS 混合」變成「純 TS」。
...
- Tracked (to `git rm`):
  - `packages/provider-codex/src/auth.c`
  - `packages/provider-codex/src/jwt.c`
  - `packages/provider-codex/src/main.c`
  - `packages/provider-codex/src/originator.c`
  - `packages/provider-codex/src/provider.c`
  - `packages/provider-codex/src/quota.c`
  - `packages/provider-codex/src/storage.c`
  - `packages/provider-codex/src/stream.c`
  - `packages/provider-codex/src/transform.c`
  - `packages/provider-codex/src/transport.c`
  - `packages/provider-codex/include/codex_provider.h`
  - `packages/provider-codex/CMakeLists.txt`
- Untracked (to `rm -rf`):
  - `packages/provider-codex/build/`
- Doc update:
