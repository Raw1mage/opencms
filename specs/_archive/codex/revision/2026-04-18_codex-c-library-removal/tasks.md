...
- [ ] 1.1 `git ls-files packages/provider-codex/` 列出所有 tracked 檔案，確認 `.c` / `.h` / `CMakeLists.txt` 哪些是 tracked
...
- [ ] 2.1 `git rm packages/provider-codex/src/{auth,jwt,main,originator,provider,quota,storage,stream,transform,transport}.c`
- [ ] 2.2 `git rm packages/provider-codex/include/codex_provider.h`（若 tracked）或 `rm -rf include/`
- [ ] 2.3 `git rm packages/provider-codex/CMakeLists.txt`
- [ ] 2.4 `rm -rf packages/provider-codex/build/`（untracked artifact）
...
- [ ] 3.3 `bun test packages/provider-codex/` 測試全綠
