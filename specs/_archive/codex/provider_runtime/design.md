...
`packages/provider-codex/` 先前保留了一整組 C 語言實作（`src/*.c`、`include/codex_provider.h`、`CMakeLists.txt`、`build/`）作為潛在的 `bun:ffi` native path，對照 `packages/opencode/src/plugin/claude-native.ts` 的 `dlopen("claude_provider.so")` 做法。

但線上實際路徑從未走過 C library：`packages/provider-codex/src/index.ts` 只 export TS 模組，`provider.ts` 以 `fetch` + `tryWsTransport` 直接走網路，整套 C 實作是 dead code。2026-04-18 診斷 subagent rotation 問題時因此誤判 `transport.c` 的 retry loop 為線上 bug，浪費半天工時。
