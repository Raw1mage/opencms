...
- **DD-1**: ~~[SUPERSEDED by DD-8 for the claude branch — claude now gets NO steer at all (factory-compatible); the ephemeral carrier below is retained only as dead reference. codex/SS branch still stands.]~~ Paralysis steering 載體依 provider class 分流,而非統一處理 — claude (SL) 走 ephemeral `<system-reminder>`(掛 sessionMessages clone tail、不落地、第三人稱),codex/其他 (SS) 維持現有持久化 role:user nudge。理由:codex 跳針靠新指令即可重算,claude 對持久化第二人稱責備會認錯脫軌並自我 few-shot 放大。完全比照既有 `evaluateSlCacheHealth` vs `evaluateSsCacheHealth` 與 CLAUDE_PROACTIVE_REMINDER 的 claude-gated 範式。
...
- **DD-8**: DD-8 (amend 2026-06-23): claude 回歸**原廠相容控制模式** — 對 claude-class session 完全跳過 paralysis 偵測→nudge→hard-halt 整個 ladder,零控制介入,loop 直接落到 generation,行為等同官方 Claude Code。實作為單一 guard:在 prompt.ts 偵測塊 `pendingSteer` 抑制之後、既有 detection else-if 之前,插入 `else if (resolvePolicy(providerId).kind === "claude") {}` 空分支。理由:(1) autonomous runner 已退役(tweaks triggerPhrases:[]),無無人看管迴圈需防;(2) 任何注入(persisted 或 ephemeral)都會毒 claude 認錯反射、醫源性製造它想防的卡死(issue_20260622)。**SUPERSEDES DD-1 的 claude 分支**(claude carrier: ephemeral system-reminder → none)。必須跳整塊而非「no-op nudge + continue」:無 pendingSteer 抑制時 continue 會在未變動的 window 上立即 re-detect、永不進 generation,形成 tight loop(DD-7)。codex (SS) 不受影響,INV-0 維持。
