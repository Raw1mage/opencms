...
Use `--add-distributor` to force a distributor column even when it isn't detected in the schematic (repeatable):

```bash
python3 <skill-path>/scripts/bom_manager.py export path/to/schematic.kicad_sch -o bom/bom.csv --add-distributor mouser
```
...
- `references/part-number-conventions.md` — detailed analysis of naming patterns across real projects
