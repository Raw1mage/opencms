...
off it). Domain-specific section rosters (e.g. an ISMS four-tier skeleton) are a _second_
...
Do **not** invoke this skill for trivial single-paragraph edits or plain-text dumps — a direct one-shot edit is simpler.
...
5. Create or edit the target document package using A's shape as the initial `blueprint.md`; write `body.md` from the binding spec and selected package materials — A and B are _only_ references, never the source of truth.
...
track by _where the visual design comes from_. All four end in an editable
...
body layouts). To build a new deck that keeps the brand frame _and_ looks
...
_eye_ is the multimodal model reading a rendered raster; the rubric is the
...
Loop (authoritative view = the .pptx a client actually gets): 0. **Objective gate**: run `docxmcp_pptx_canvas_lint` (canvas / template-bank:
...
it as the authoritative _deliverable_ view, not a fidelity guarantee.
...
resolves single-file _and_ staged/tarball tokens to the lone PDF, so the token
...
- Verify the produced file with docxmcp (`docxmcp_document` action=probe) or LibreOffice headless to confirm front matter order, heading/list `numPr`, paragraph/table counts, and TOC field presence.
...
There is **no WebDAV mount**. Instead the boundary is a _session-handle facade_
...
- **Single file** → `POST /files` (multipart). The token carries that one file.
  ...
  _client_ path as the facade's `path` arg and the dispatcher uploads it and
...
- **Multiple files / a whole package** → `POST /files` with a tarball
  ...
  `.docxmcp-meta.json`.
- **Tool output** → decompose / assemble / render write their artefacts _into
  the same token directory_, so the workspace accretes the src package and
