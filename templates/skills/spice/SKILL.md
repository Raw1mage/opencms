...
```bash
# Recommended: integrate into the current run folder (writes sim report + spice_work/ under the run, tracked by the manifest)
python3 <skill-path>/scripts/simulate_subcircuits.py analysis/<run_id>/schematic.json --analysis-dir analysis/

# Simulate all supported subcircuit types (one-off output file)
...
| `scripts/spice_tolerance.py` | Internal helper — Monte Carlo tolerance resolution, randomized sampling, and sensitivity aggregation (no CLI; used by `simulate_subcircuits.py --monte-carlo`) |
