...
def build_report(simulation_runs):
    ...
    try:
        ...
        # kicad analysis engine now lives under the bodesign skill; fall back
        # to a standalone kicad skill if one is still present.
        _here = _os.path.dirname(_os.path.abspath(__file__))
        for _kicad_scripts in (_os.path.join(_here, '..', '..', 'bodesign', 'engines', 'kicad', 'scripts'),
                               _os.path.join(_here, '..', '..', 'kicad', 'scripts')):
