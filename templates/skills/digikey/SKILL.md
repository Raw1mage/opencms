...
Use `sync_datasheets_digikey.py` to maintain a `datasheets/` directory alongside a KiCad project. It extracts components from the schematic, searches DigiKey for datasheet URLs, downloads missing PDFs, and writes a `manifest.json`. Subsequent runs are incremental — only new or changed parts are fetched.
...
# Re-download everything, even files already present
python3 <skill-path>/scripts/sync_datasheets_digikey.py <file.kicad_sch> --force-all
...
**MPN-list batch mode** — when you have a list of MPNs but no KiCad
...
- **Writes `manifest.json`** — maps each MPN to its PDF file, manufacturer, description, download status, and URL. The kicad skill reads this during design review to cross-reference datasheets with the schematic.
