...
These are import-only modules (no `__main__`/argparse) — other skills import them via `sys.path` injection rather than running them from the shell.
...
- `scripts/datasheet_verify.py` — cross-check extraction vs schematic usage. Depends on `parse_value` from the kicad skill's `kicad_utils.py`, so it only runs in the kicad analysis context.
- `scripts/datasheet_features.py` — consumer helper API (new in v2, EXTRACTION_VERSION=2)
