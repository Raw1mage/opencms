...
| Max board size | up to ~600x1200mm (confirm in quote tool) (1-2 layer) |
...
| Via diameter | see quote tool |
| Via drill | 0.15mm |
| Annular ring | see quote tool |
| Layer count | Up to 14 layers (standard), 24+ (advanced) |
| Max board size | up to ~560x1150mm (confirm in quote tool) |
...
- **Rotation offsets** — PCBWay's pick-and-place rotation conventions may differ from KiCad and from JLCPCB's offsets; PCBWay does not publish a fixed offset table. Don't assume JLCPCB's corrections carry over. Verify pin-1 orientation per footprint against PCBWay's assembly drawings/file review before each order, and supply annotated assembly drawings for polarized or orientation-sensitive parts.
