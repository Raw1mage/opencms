...
interface ArtifactCheck {
  ...
  minTasks?: boolean;
  customCheck?: (body: string, state: State) => string[];
  optional?: boolean;
}
...
const TASKS: ArtifactCheck = {
  ...
  customCheck: (body, state) => {
    ...
    if (checklist.length === 0)
      issues.push("must have at least one checklist item");
    if (["planned", "implementing"].includes(state) && unchecked.length === 0)
      issues.push("must have at least one unchecked `- [ ]` checklist item");
    if (
      ["verified", "living", "archived"].includes(state) &&
      unchecked.length > 0
    )
      issues.push("must not have unchecked `- [ ]` checklist items");
    return issues;
  },
};
...
const IDEF0: ArtifactCheck = {
  ...
  customCheck: (body) => {
    ...
    if (!/^A\d+$/.test(obj.node_reference))
      issues.push("missing or invalid node_reference");
...
// C4 is opt-in: validated only when c4.json exists. Listing it in REQUIREMENTS
// triggers structural lint on present files; missing file is not a blocker.
// Rationale: miatdiagram skill §4.1 deprecated default c4 in favor of block/stack
// Module Architecture diagrams; per SKILL.md §12.1 c4 is optional for doc/reverse-eng specs.
...
function validateArtifact(
  ...
  if (check.kind === "markdown") {
    if (check.requiredHeadings) {
      const missing = check.requiredHeadings.filter(
        (h) => !extractSection(body, h),
      );
      if (missing.length > 0)
  ...
  if (check.customCheck) {
    issues.push(...check.customCheck(body, checkState));
  }
...
function main(): void {
  ...
  for (let i = 0; i < rawArgs.length; i++) {
    ...
    else if (arg.startsWith("--as-state="))
      asStateOverride = arg.slice("--as-state=".length) as State;
  ...
  const noteOverride = asStateOverride
    ? ` (override from actual state=${state.state})`
    : "";
  console.error(
    `plan-validate: checking ${requirements.length} artifact(s) for state=${checkState}${noteOverride}`,
  );

  const issues: string[] = [];
  for (const req of requirements) {
    issues.push(...validateArtifact(finalPath, req, checkState));
  }

  if (issues.length > 0) {
    console.error(
      `FAIL — ${issues.length} blocker(s) for state=${checkState}:\n`,
    );
  ...
  console.log(
    `PASS — all ${requirements.length} artifact(s) required for state=${checkState} are valid.`,
  );
}
