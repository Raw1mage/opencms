...
# Configuration — determine PROJECT_ROOT
# Priority: 1) $PWD if it's a source repo  2) script's own directory  3) standalone fallback
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

if [ -f "${PWD}/packages/opencode/src/index.ts" ]; then
    # Caller's working directory IS a source repo — use it
    PROJECT_ROOT="${PWD}"
elif [ -f "${SCRIPT_DIR}/packages/opencode/src/index.ts" ]; then
    # Script lives inside a source repo
...
load_server_cfg() {
    ...
    # In source-repo mode, keep repo-local dist; only override for standalone
