...
- [ ] 1.7 Wire into codex.ts fetch interceptor: if WS connect succeeds, use WS path; else fall through to HTTP
- [ ] 1.8 End-to-end validation: send "Say hello" via WS → verify text output appears in UI
...
- [ ] 2.11 Implement retry budget: WS connect fail → retry once → HTTP fallback
- [ ] 2.12 Implement stream retry: budget-limited, emit "Reconnecting N/N" feedback
- [ ] 2.13 Validate: WS error → HTTP fallback → session continues
- [ ] 2.14 Validate: account rotation → new WS connection with correct auth

## 3. Phase 3 — Incremental Delta

- [ ] 3.1 Capture response_id from response.completed event, store in session state
- [ ] 3.2 Implement `getIncrementalItems()`: compare current vs last request, detect prefix match
- [ ] 3.3 Implement `prepareWsRequest()`: if prefix match → send delta items + previous_response_id; else full request
- [ ] 3.4 Implement cache eviction on 4xx/5xx: clear previous_response_id
- [ ] 3.5 Implement `previous_response_not_found` handling: reset to full context
- [ ] 3.6 Validate: second turn input items < first turn (confirmed via DIAG trace)
...
> Phase 4 is shelved per prior product decision. Only execute if Phases 1-3 are stable and user explicitly requests.
