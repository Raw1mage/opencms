...
### 1. `bun run verify:typecheck` cannot find `turbo` — RESOLVED (2026-05-30)
...
- **Resolution**: `node_modules/.bin/turbo` now present (deps reinstalled). Verified 2026-05-30.

### 2. Repo-wide `tsc --noEmit` blocked by plan-builder template syntax errors — RESOLVED (2026-05-30)
...
- **Root cause**: line 4 JSDoc comment contained the literal `refactor-*/` — the `*/` prematurely terminated the block comment, so the trailing ` snapshot, undoing a refactor operation.` was parsed as code (`SyntaxError: Expected ";"`).
- **Fix**: rewrote the comment path token to `.history/refactor-<date>/` (no `*/` sequence). Verified via `bun --check` — now parses cleanly (exits 2 = usage error, i.e. runs to arg-validation, no longer a SyntaxError).
