...
- **Status**: FIXED（2026-06-17）— root cause 在 `provider-claude/src/antml-salvage.ts:57-65`（值被當字串搶救）；修復落點選在 `mcp/index.ts` `convertMcpTool` execute 單一 chokepoint 做 schema-based coercion。回歸測試 15 cases 全綠、typecheck 乾淨。**未 commit、未端到端 restart 驗證**。
