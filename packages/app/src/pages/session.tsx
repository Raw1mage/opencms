...
export default function Page() {
  ...
  // ── Render watchdog (auto-rescue) ──────────────────────────────────
  // Symptom we keep hitting: assistant replies are in the sync store
  // (diag confirms textLen > 0, finish=stop) but UI doesn't show them.
  // Two failure modes both produce this:
  //   (a) Solid skipped rendering (reducer drift, stale slice, keyed
  //       <For> filtering wrong) → DOM element missing for store msg
  //   (b) Solid rendered but the element has 0 height / hidden ancestor
  //       (CSS / lazyload window collapsed, scroll-spy bug)
  // The watchdog scans the last N messages every few seconds. If any
  // store message has no visible DOM presence (case a OR b), we
  // schedule a one-shot self-rescue: reset turnStart to 0 (forces full
  // render of every turn) and force-resync the session (fresh fetch +
  // remount). Bounded by a per-session retry counter so a stuck render
  // can't loop forever.
  const WATCHDOG_INTERVAL_MS = 3000
  const WATCHDOG_TAIL_SAMPLE = 8
  const WATCHDOG_MAX_RESCUES = 2
  let watchdogRescuesThisSession = 0
  let watchdogLastSessionID: string | undefined
  createEffect(() => {
    const id = params.id
    if (!id) return
    if (typeof document === "undefined") return
    if (id !== watchdogLastSessionID) {
      watchdogLastSessionID = id
      watchdogRescuesThisSession = 0
    }
    const timer = window.setInterval(() => {
      if (params.id !== id) return
      const msgs = messages()
      if (msgs.length === 0) return
      const tail = msgs.slice(-WATCHDOG_TAIL_SAMPLE)
      let missing = 0
      let zeroHeight = 0
      for (const m of tail) {
        ...
        const el =
          ...
          document.querySelector(`[data-message="${mid}"]`)
        if (!el) {
          missing++
          continue
        }
        if ((el as HTMLElement).offsetHeight === 0) zeroHeight++
      }
      if (missing === 0 && zeroHeight === 0) return
      if (watchdogRescuesThisSession >= WATCHDOG_MAX_RESCUES) return
      watchdogRescuesThisSession++
      console.warn("[render-watchdog] tail not fully rendered — auto-rescue", {
        sessionID: id,
        sampled: tail.length,
        missing,
        zeroHeight,
        rescueAttempt: watchdogRescuesThisSession,
      })
      sendSessionReloadDebugBeacon({
        sdk,
        event: "render-watchdog:rescue",
        sessionID: id,
        payload: {
          sampled: tail.length,
          missing,
      ...
      // Step 1: reveal everything (no slice). Cheap; fixes case (a)
      // when turnStart was stuck past the end.
      setStore("turnStart", 0)
      // Step 2: force a full session resync to remount messages with
      // fresh data — fixes case (b) where the DOM is stale.
