...
3.  **說明型輸出的主動閱讀節奏控制 (`read-mode`)**：
    - 當回覆的主要目的，是**回答使用者提問並幫助其理解**（例如架構解釋、設計權衡、教學、長篇分析、文件導讀），就應主動考慮載入 `skill(name="read-mode")`，不必等到內容非常長才啟用。
    - 啟用後，應將內容拆成自然段落，並以**約 20 行上下**作為停頓節奏；每輸出一段就使用 `question` 提供至少 `是`、`否` 選項，並允許使用者輸入其他意見。
