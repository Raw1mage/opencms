...
| `src/project/workspace/*`         | **Workspace Kernel (Phase 1).** Defines workspace aggregate types, directory→workspace resolution, attachment ownership contracts, registry interface, runtime workspace service façade, and minimal lifecycle transition contract. | `resolveWorkspace`, `WorkspaceRegistry`, `WorkspaceService`, workspace schemas/types | **In:** Project info + directory + session/pty events<br>**Out:** Workspace aggregate / registry/service/lifecycle lookups |
