...
| `src/server/routes/workspace.ts`    | **Workspace Routes.** API boundary for workspace list/current/status/read plus lifecycle state transitions and the first runtime-owned reset operation (`reset-run`) backed by workspace service/operations. | `WorkspaceRoutes`    | **In:** Instance project/workspace<br>**Out:** Workspace list/read/status/lifecycle/operation |
