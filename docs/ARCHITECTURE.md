...
| `src/project/workspace/*`         | **Workspace Kernel (Phase 1).** Defines workspace aggregate types, directory→workspace resolution, attachment ownership contracts, registry interface, runtime workspace service façade, minimal lifecycle transition contract, and now worker attachment registration. | `resolveWorkspace`, `WorkspaceRegistry`, `WorkspaceService`, workspace schemas/types | **In:** Project info + directory + session/pty/worker events<br>**Out:** Workspace aggregate / registry/service/lifecycle lookups |
