...
| `src/project/workspace/*`         | **Workspace Kernel (Phase 1).** Defines workspace aggregate types, directory→workspace resolution, attachment ownership contracts, and registry interface for future workspace-scoped runtime evolution. | `resolveWorkspace`, `WorkspaceRegistry`, workspace schemas/types | **In:** Project info + directory<br>**Out:** Workspace aggregate / registry lookups |
...
3. **Why this matters**
   ...
   - This is still an intermediate phase: app currently uses an adapter-local workspace helper, while runtime owns the canonical workspace kernel under `packages/opencode/src/project/workspace/*`.
