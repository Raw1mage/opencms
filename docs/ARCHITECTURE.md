...
- Smart Runner traces may also carry bounded `suggestion` metadata for non-authoritative route shaping. The first unlocked suggestion type is `replan`, which is surfaced in session inspection/history only and does not mutate todos or override deterministic control flow.
